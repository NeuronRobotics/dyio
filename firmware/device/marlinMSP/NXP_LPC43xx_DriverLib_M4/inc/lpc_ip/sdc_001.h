...
typedef struct {
	...
	__IO uint32_t DATATIMER;
	__IO uint32_t DATALENGTH;
...
#define SDC_COMMAND_NO_RSP          ((uint32_t) 0)
 ...
 */
typedef enum {
	SDC_NO_RESPONSE = SDC_COMMAND_NO_RSP,
	SDC_SHORT_RESPONSE = SDC_COMMAND_SHORT_RSP,
	SDC_LONG_RESPONSE = SDC_COMMAND_LONG_RSP,
 ...
 */
typedef enum {
	SDC_TRANSFER_DIR_FROMCARD = SDC_DATACTRL_DIR_FROMCARD,
	SDC_TRANSFER_DIR_TOCARD = SDC_DATACTRL_DIR_TOCARD,
 ...
 */
typedef enum {
	SDC_TRANSFER_MODE_STREAM = SDC_DATACTRL_XFER_MODE_STREAM,
	SDC_TRANSFER_MODE_BLOCK = SDC_DATACTRL_XFER_MODE_BLOCK,
...
/**
 * @brief SDC Command structure
 */
typedef struct {
	uint8_t CmdIndex;			/*!< Command Index */
	uint32_t Argument;			/*!< Command Argument*/
	IP_SDC_001_RESPONSE_T RespType;	/*!< Response Type*/
	uint32_t Flag;				/*!< Command Flag. It is or-ed bit value of SDC_COMMAND_INTERRUPT and SDC_COMMAND_PENDING */
} IP_SDC_001_CMD_Type;
...
} IP_SDC_001_DATA_TRANSFER_Type;

/**
 * @brief	Control power of SDC peripheral
...
/**
 * @brief	Control clock of SDC peripheral
 * @param	pSDC	: Pointer to SDC register block
...
/**
 * @brief	Set SDC Command Info
 * @param	pSDC	: Pointer to SDC register block
 * @param	pCmd	: Command Information
 * @return	Nothing
 */
 ...
 * @return	Nothing
 */
STATIC INLINE void IP_SDC_SetDataTimer(IP_SDC_001_Type *pSDC, uint32_t timeout)
{
	pSDC->DATATIMER = timeout;
}

/**
 * @brief	Set SDC Data Transfer Info
 * @param	pSDC		: Pointer to SDC register block
 * @param	pTransfer	: Transfer Information
 * @return	Nothing
 */
 ...
 * @param	pSDC		: Pointer to SDC register block
 * @param	pSrc		: The buffer hold the data need to write to card
 * @param	bFirstHalf	: true (write to the first half of FIFO) false (write to the second half of FIFO)
 * @return	Nothing
 */
void IP_SDC_WriteFIFO(IP_SDC_001_Type *pSDC, uint32_t *pSrc, bool bFirstHalf);
 ...
 * @return	Nothing
 */
void IP_SDC_ReadFIFO(IP_SDC_001_Type *pSDC, uint32_t *pDst, bool bFirstHalf);
 ...
 */
STATIC INLINE uint32_t IP_SDC_GetStatus(IP_SDC_001_Type *pSDC)
{
	return pSDC->STATUS;
}
 ...
 * @param	pSDC	: Pointer to SDC register block
 * @param	flag	: flag to be cleared (Or-ed bit value of SDC_CLEAR_*)
 * @return	None
 */
STATIC INLINE void IP_SDC_ClearStatus(IP_SDC_001_Type *pSDC, uint32_t flag)
{
	pSDC->CLEAR = flag;
}
 ...
 * @return	None
 */
STATIC INLINE void IP_SDC_SetIntMask(IP_SDC_001_Type *pSDC, uint32_t mask)
{
	pSDC->MASK0 = mask;
}
 ...
 * @return	None
 */
STATIC INLINE void IP_SDC_DeInit(IP_SDC_001_Type *pSDC)
{
