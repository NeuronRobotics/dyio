...
STATIC void getSectionAddress(IP_CAN_001_AF_Type *pCanAF,
							  IP_CANAF_RAM_SECTION SectionID, uint16_t *StartAddr, uint16_t *EndAddr)
{
...
STATIC INLINE uint16_t getTotalEntryNum(IP_CAN_001_AF_Type *pCanAF)
{
...
STATIC INLINE void setSectionEndAddress(IP_CAN_001_AF_Type *pCanAF,
										IP_CANAF_RAM_SECTION SectionID, uint16_t EndAddr)
{
	pCanAF->ENDADDR[SectionID] = CANAF_ENDADDR(EndAddr);
}

/* Get information of the received frame. Return ERROR means no message came.*/
STATIC INLINE Status getReceiveFrameInfo(IP_CAN_001_Type *pCAN,
										 IP_CAN_001_RX_Type *pRxFrame)
{
	*pRxFrame = pCAN->RX;
...
STATIC INLINE void setSendFrameInfo(IP_CAN_001_Type *pCAN, IP_CAN_BUFFER_ID TxBufID,
									IP_CAN_001_TX_Type *pTxFrame)
{
	pCAN->TX[TxBufID] = *pTxFrame;
}

/* Create the standard ID entry */
STATIC uint16_t createStdIDEntry(IP_CAN_STD_ID_Entry_Type *pEntryInfo, bool IsFullCANEntry)
{
	uint16_t Entry = 0;
...
STATIC void readStdIDEntry(uint16_t EntryVal, IP_CAN_STD_ID_Entry_Type *pEntryInfo)
{
...
STATIC Status setupSTDSection(uint32_t *pCanAFRamAddr,
							  IP_CAN_STD_ID_Entry_Type *pStdCANSec,
							  uint16_t EntryNum,
							  bool IsFullCANEntry)
{
	uint16_t i;
...
STATIC Status setupSTDRangeSection(uint32_t *pCanAFRamAddr,
								   IP_CAN_STD_ID_RANGE_Entry_Type *pStdRangeCANSec,
								   uint16_t EntryNum)
{
	return setupSTDSection(pCanAFRamAddr, (IP_CAN_STD_ID_Entry_Type *) pStdRangeCANSec, EntryNum * 2, false);
}
...
STATIC uint32_t createExtIDEntry(IP_CAN_EXT_ID_Entry_Type *pEntryInfo)
{
	uint32_t Entry = 0;
...
STATIC void readExtIDEntry(uint32_t EntryVal, IP_CAN_EXT_ID_Entry_Type *pEntryInfo)
{
...
STATIC Status setupEXTSection(uint32_t *pCanAFRamAddr, IP_CAN_EXT_ID_Entry_Type *pExtCANSec, uint16_t EntryNum)
{
	uint16_t i;
	uint32_t CurID = 0;
	uint32_t Entry;
...
STATIC Status setupEXTRangeSection(uint32_t *pCanAFRamAddr,
								   IP_CAN_EXT_ID_RANGE_Entry_Type *pExtRangeCANSec,
								   uint16_t EntryNum)
{
	return setupEXTSection(pCanAFRamAddr, (IP_CAN_EXT_ID_Entry_Type *) pExtRangeCANSec, EntryNum * 2);
}
...
void IP_CAN_Init(IP_CAN_001_Type *pCAN) {
	volatile uint32_t i;
	/* Enter to Reset Mode */
	pCAN->MOD = CAN_MOD_RM;
...
/* Set CAN Mode */
void IP_CAN_SetMode(IP_CAN_001_Type *pCAN, uint32_t Mode, FunctionalState NewState)
{
...
/* Set Bus Timing */
void IP_CAN_SetBusTiming(IP_CAN_001_Type *pCAN, IP_CAN_BUS_TIMING_Type *pBusTiming) {
	/* Enter to Reset Mode */
...
/* Receive CAN Message */
Status IP_CAN_Receive(IP_CAN_001_Type *pCAN, IP_CAN_MSG_Type *pMsg) {
	int8_t i;
	IP_CAN_001_RX_Type RxFrame;
...
/* Send CAN Message */
Status IP_CAN_Send(IP_CAN_001_Type *pCAN, IP_CAN_BUFFER_ID TxBufID, IP_CAN_MSG_Type *pMsg)
{
	uint8_t i = 0;
	IP_CAN_001_TX_Type TxFrame;
...
void IP_CAN_IntConfig(IP_CAN_001_Type *pCAN, uint32_t IntMask, FunctionalState NewState) {
	if (NewState == ENABLE) {
		pCAN->IER |= IntMask;
	}
	else {
...
void IP_CAN_FullCANIntConfig(IP_CAN_001_AF_Type *pCanAF, FunctionalState NewState)
{
...
uint32_t IP_CAN_GetFullCANIntStatus(IP_CAN_001_AF_Type *pCanAF, uint8_t ObjID)
{
	if (ObjID < 64) {
...
Status IP_CAN_FullCANReceive(IP_CAN_001_AF_Type *pCanAF, IP_CAN_001_AF_RAM_Type *pCanAFRam
							 , uint8_t ObjID, IP_CAN_MSG_Type *pMsg, uint8_t *pSCC) {
	uint32_t    *pSrc;
...
/* Initialize CAN AF */
void IP_CAN_AF_Init(IP_CAN_001_AF_Type *pCanAF, IP_CAN_001_AF_RAM_Type *pCanAFRam) {
...
/* Clear AF LUT */
void IP_CAN_ClearAFLUT(IP_CAN_001_AF_Type *pCanAF, IP_CAN_001_AF_RAM_Type *pCanAFRam) {
	uint32_t i = 0;
	uint32_t CurMode = IP_CAN_AF_GetMode(pCanAF);
...
/* Set AF Lookup Table */
Status IP_CAN_SetAFLUT(IP_CAN_001_AF_Type *pCanAF, IP_CAN_001_AF_RAM_Type *pCanAFRam,
					   IP_CAN_AF_LUT_Type *pAFSections) {
...
uint16_t IP_CAN_GetEntriesNum(IP_CAN_001_AF_Type *pCanAF, IP_CAN_001_AF_RAM_Type *pCanAFRam,
							  IP_CANAF_RAM_SECTION SectionID)
{
...
Status IP_CAN_InsertSTDEntry(IP_CAN_001_AF_Type *pCanAF,
							 IP_CAN_001_AF_RAM_Type *pCanAFRam,
							 IP_CAN_STD_ID_Entry_Type *pEntry,
							 bool IsFullCANEntry)
{
	int32_t IDIndex = 0;
	uint16_t StartRow, EndRow;
	...
	if ((IDIndex % 2) == 0) {
		/* Insert new item */
		pCanAFRam->MASK[StartRow + IDIndex / 2] &= 0x0000FFFF;
		pCanAFRam->MASK[StartRow + IDIndex / 2] |= tmp << 16;

		/* Insert unused item if the new item is the last item*/
		if (IDIndex == EntryCnt) {
			pCanAFRam->MASK[StartRow + IDIndex / 2] &= 0xFFFF0000;
			pCanAFRam->MASK[StartRow + IDIndex / 2] |= createUnUsedSTDEntry(pEntry->CtrlNo);
		}
	}
	else {
		/* In case the new entry is not the last item, shift the item at the found index to the next row*/
		if (IDIndex < EntryCnt ) {
			pCanAFRam->MASK[StartRow + IDIndex / 2 + 1] &= 0x0000FFFF;
			pCanAFRam->MASK[StartRow + IDIndex / 2 + 1] |= (pCanAFRam->MASK[StartRow + IDIndex / 2] & 0xFFFF) << 16;
		}
		pCanAFRam->MASK[StartRow + IDIndex / 2] &= 0xFFFF0000;
		pCanAFRam->MASK[StartRow + IDIndex / 2] |= tmp;
	}
...
/* Insert an entry into FullCAN Table */
Status IP_CAN_InsertFullCANEntry(IP_CAN_001_AF_Type *pCanAF,
								 IP_CAN_001_AF_RAM_Type *pCanAFRam,
								 IP_CAN_STD_ID_Entry_Type *pEntry) {
...
Status IP_CAN_InsertIndividualSTDEntry(IP_CAN_001_AF_Type *pCanAF,
									   IP_CAN_001_AF_RAM_Type *pCanAFRam,
									   IP_CAN_STD_ID_Entry_Type *pEntry)
{
...
Status IP_CAN_InsertIndividualEXTEntry(IP_CAN_001_AF_Type *pCanAF,
									   IP_CAN_001_AF_RAM_Type *pCanAFRam,
									   IP_CAN_EXT_ID_Entry_Type *pEntry) {
	int32_t IDIndex = 0;
	uint16_t StartRow, EndRow;
	uint16_t EntryCnt = 0;
	uint16_t i = 0;
...
Status IP_CAN_InsertGroupSTDEntry(IP_CAN_001_AF_Type *pCanAF,
								  IP_CAN_001_AF_RAM_Type *pCanAFRam,
								  IP_CAN_STD_ID_RANGE_Entry_Type *pEntry)
{
	...
	if (InsertIndex > 0) {
		...
		if (pEntry->UpperID.ID_11 < LowerID) {
			if (pEntry->LowerID.ID_11 < PrevUpperID) {	/* The new range is merged to the range of the previous row */
				pCanAFRam->MASK[InsertIndex - 1] &= 0xFFFF0000;	/* Clear upper entry */
				pCanAFRam->MASK[InsertIndex - 1] |= createStdIDEntry(&pEntry->UpperID, false);
				return SUCCESS;
			}
			else {
	...
	if ((EndRow) && (InsertIndex <= EndRow)) {
		if (pEntry->UpperID.ID_11 >= UpperID) {	/* The new range is merged to the range of the next row */
			...
			if (pEntry->UpperID.ID_11 > LowerID) {	/* The new range is merged to the range of the next row */
				pCanAFRam->MASK[InsertIndex] &= 0x0000FFFF;	/* Clear lower entry */
				pCanAFRam->MASK[InsertIndex] |= createStdIDEntry(&pEntry->LowerID, false) << 16;
				return SUCCESS;
			}
		}
...
Status IP_CAN_InsertGroupEXTEntry(IP_CAN_001_AF_Type *pCanAF,
								  IP_CAN_001_AF_RAM_Type *pCanAFRam,
								  IP_CAN_EXT_ID_RANGE_Entry_Type *pEntry)
{
...
Status IP_CAN_RemoveSTDEntry(IP_CAN_001_AF_Type *pCanAF,
							 IP_CAN_001_AF_RAM_Type *pCanAFRam,
							 int16_t IDIndex,
							 bool IsFullCANEntry)
{
...
Status IP_CAN_RemoveLUTEntry(IP_CAN_001_AF_Type *pCanAF,
							 IP_CAN_001_AF_RAM_Type *pCanAFRam,
							 IP_CANAF_RAM_SECTION SectionID,
							 int16_t Position)
{
...
Status IP_CAN_RemoveFullCANEntry(IP_CAN_001_AF_Type *pCanAF,
								 IP_CAN_001_AF_RAM_Type *pCanAFRam,
								 int16_t Position) {
...
Status IP_CAN_RemoveIndividualSTDEntry(IP_CAN_001_AF_Type *pCanAF,
									   IP_CAN_001_AF_RAM_Type *pCanAFRam,
									   int16_t Position)
{
...
Status IP_CAN_RemoveGroupSTDEntry(IP_CAN_001_AF_Type *pCanAF,
								  IP_CAN_001_AF_RAM_Type *pCanAFRam,
								  int16_t Position)
{
...
Status IP_CAN_RemoveIndividualEXTEntry(IP_CAN_001_AF_Type *pCanAF,
									   IP_CAN_001_AF_RAM_Type *pCanAFRam,
									   int16_t Position) {
...
Status IP_CAN_RemoveGroupEXTEntry(IP_CAN_001_AF_Type *pCanAF,
								  IP_CAN_001_AF_RAM_Type *pCanAFRam,
								  int16_t Position)
{
...
/* Read STD Entry */
Status IP_CAN_ReadSTDEntry(IP_CAN_001_AF_Type *pCanAF,
						   IP_CAN_001_AF_RAM_Type *pCanAFRam,
						   uint16_t Position,
						   bool   IsFullCANEntry,
						   IP_CAN_STD_ID_Entry_Type *pEntry)
...
Status IP_CAN_ReadFullCANEntry(IP_CAN_001_AF_Type *pCanAF,
							   IP_CAN_001_AF_RAM_Type *pCanAFRam,
							   uint16_t Position,
							   IP_CAN_STD_ID_Entry_Type *pEntry)
{
...
Status IP_CAN_ReadIndividualSTDEntry(IP_CAN_001_AF_Type *pCanAF,
									 IP_CAN_001_AF_RAM_Type *pCanAFRam,
									 uint16_t Position,
									 IP_CAN_STD_ID_Entry_Type *pEntry)
{
...
Status IP_CAN_ReadGroupSTDEntry(IP_CAN_001_AF_Type *pCanAF,
								IP_CAN_001_AF_RAM_Type *pCanAFRam,
								uint16_t Position,
								IP_CAN_STD_ID_RANGE_Entry_Type *pEntry)
{
...
Status IP_CAN_ReadIndividualEXTEntry(IP_CAN_001_AF_Type *pCanAF,
									 IP_CAN_001_AF_RAM_Type *pCanAFRam,
									 uint16_t Position,
									 IP_CAN_EXT_ID_Entry_Type *pEntry)
{
...
Status IP_CAN_ReadGroupEXTEntry(IP_CAN_001_AF_Type *pCanAF,
								IP_CAN_001_AF_RAM_Type *pCanAFRam,
								uint16_t Position,
								IP_CAN_EXT_ID_RANGE_Entry_Type *pEntry)
