 ...
 * @return	None
 */
STATIC INLINE void Chip_WWDT_DeInit(void)
{
	IP_WWDT_DeInit(LPC_WWDT);
}
...
{
	IP_WWDT_SetTimeOut(LPC_WWDT, timeout);
}
...
{
	IP_WWDT_SetWarning(LPC_WWDT, timeout);
}
...
{
	IP_WWDT_SetWindow(LPC_WWDT, timeout);
}
...
{
	IP_WWDT_SetOption(LPC_WWDT, options);
}
...
{
	IP_WWDT_UnsetOption(LPC_WWDT, options);
}
...
{
	return IP_WWDT_GetStatus(LPC_WWDT);
}
 ...
 * @return	None
 */
STATIC INLINE void Chip_WWDT_ClearStatusFlag(uint32_t status)
{
	IP_WWDT_ClearStatusFlag(LPC_WWDT,status);
}
 ...
 */
STATIC INLINE uint32_t Chip_WWDT_GetCurrentCount(void)
{
	return IP_WWDT_GetCurrentCount(LPC_WWDT);
}
