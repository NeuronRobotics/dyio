...
#if defined(CHIP_LPC175X_6X) || defined(CHIP_LPC177X_8X) || defined(CHIP_LPC407X_8X) || defined(CHIP_LPC18XX) || defined(CHIP_LPC43XX)
	if (UARTPort == 1) {
#endif
...
void IP_UART_ABCmd(IP_USART_001_Type *LPC_UART, UART_AB_CFG_Type *ABConfigStruct, FunctionalState NewState)
{
	if (NewState == ENABLE)
	{
	...
	/* FDR value must be reset to default value */
        ...
        {
        	LPC_UART->ACR = UART_ACR_START | UART_ACR_MODE;
        }

        if (ABConfigStruct->AutoRestart == ENABLE)
        {
        	LPC_UART->ACR = UART_ACR_START | UART_ACR_AUTO_RESTART;
        }
	}
	else
	{
...
void IP_UART_ABClearIntPending(IP_USART_001_Type *LPC_UART, UART_INT_STATUS_Type ABIntType)
{
	LPC_UART->ACR |= ABIntType;
}
