...
static void CCAN_IF_Buf_transfer(IP_CCAN_001_Type *pCCAN, msg_interface_t IFsel, uint32_t msg_num, uint8_t direction) {
	msg_num &= 0x3F;
...
void IP_CCAN_SWInit(IP_CCAN_001_Type *pCCAN, FunctionalState NewState)
{
	if (NewState == ENABLE) {
...
void IP_CCAN_TimingCfg(IP_CCAN_001_Type *pCCAN,
					   uint32_t ClkDiv,
...
void IP_CCAN_IntEnable(IP_CCAN_001_Type *pCCAN, IP_CCAN_Int_t Int_type, FunctionalState NewState)
{
	if (NewState == ENABLE) {
		pCCAN->CNTL |= Int_type;
	}
...
void IP_CCAN_AutoRetransmitEnable(IP_CCAN_001_Type *pCCAN, FunctionalState NewState)
{
	if (NewState == ENABLE) {
...
uint8_t IP_CCAN_GetErrCounter(IP_CCAN_001_Type *pCCAN, IP_CCAN_TRxMode_t TRMode)
{
...
uint32_t IP_CCAN_GetStatus(IP_CCAN_001_Type *pCCAN)
{
	return pCCAN->STAT;
}

/* Set the CCAN status */
void IP_CCAN_SetStatus(IP_CCAN_001_Type *pCCAN, uint32_t val)
{
	pCCAN->STAT = val & 0x1F;
}

/* Get the source ID of an interrupt */
uint32_t IP_CCAN_Get_IntID(IP_CCAN_001_Type *pCCAN)
{
	return pCCAN->INT;
}

/* Enable/Disable test mode in CCAN */
void IP_CCAN_TestModeEnable(IP_CCAN_001_Type *pCCAN, IP_CCAN_TestMode_t test_mode, FunctionalState NewState)
{
	if (NewState == ENABLE) {
...
void IP_CCAN_ClearIntPend(IP_CCAN_001_Type *pCCAN, msg_interface_t IFsel, uint8_t msg_num)
{
	msg_num &= 0x3F;
...
void IP_CCAN_Clear_NewDataFlag(IP_CCAN_001_Type *pCCAN, msg_interface_t IFsel, uint8_t msg_num)
{
	msg_num &= 0x3F;
...
void IP_CCAN_SetValidMsg(IP_CCAN_001_Type *pCCAN, msg_interface_t IFsel, uint8_t msg_num, FunctionalState NewState)
{
...
void IP_CCAN_GetMsgObject(IP_CCAN_001_Type *pCCAN, msg_interface_t IFsel, uint8_t msg_num, message_object *msg_buf)
{
	uint32_t *temp_data;
	if (!msg_buf) {
		return;
