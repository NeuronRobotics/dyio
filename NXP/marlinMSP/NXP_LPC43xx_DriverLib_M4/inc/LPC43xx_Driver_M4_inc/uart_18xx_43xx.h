 ...
 *              - UART_INTSTS_RTS
 */
UART_INT_STATUS_Type Chip_UART_GetIntStatus(LPC_USART_Type *UARTx);
 ...
 * @param	UARTx		: Pointer to selected UARTx peripheral
 * @param	ABConfigStruct  : A pointer to UART_AB_CFG_Type structure that
 ...
 */
STATIC INLINE void Chip_UART_ABCmd(LPC_USART_Type *UARTx, UART_AB_CFG_Type *ABConfigStruct, FunctionalState NewState)
{
	IP_UART_ABCmd(UARTx, ABConfigStruct, NewState);
}
 ...
 * @return	Nothing
 */
STATIC INLINE void Chip_UART_ABClearIntPending(LPC_USART_Type *UARTx, UART_INT_STATUS_Type ABIntType)
{
	IP_UART_ABClearIntPending(UARTx, ABIntType);
}

/* UART interrupt service routine */
FlagStatus Chip_UART_GetABEOStatus(void);
