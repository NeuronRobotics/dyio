...
int Chip_DMA_InitChannelCfg(GPDMA_Channel_CFG_Type *GPDMACfg,
		uint8_t  ChannelNum,
		uint32_t src,
		uint32_t dst,
		uint32_t Size,
...
Status Chip_DMA_Transfer(uint8_t ChannelNum,
		uint32_t src,
		uint32_t dst,
		FlowControlType TransferType,
		uint32_t Size)
{
	...
	ret = Chip_DMA_InitChannelCfg(&GPDMACfg, ChannelNum, src, dst, Size, TransferType);
	if (ret < 0)
		return ERROR;
	...
	if (IP_GPDMA_Setup(LPC_GPDMA, &GPDMACfg, cwrd, 0, SrcPeripheral, DstPeripheral) == ERROR)
		return ERROR;
...
Status Chip_DMA_PrepareDescriptor(DMA_TransferDescriptor_t *DMADescriptor,
		uint32_t src,
		uint32_t dst,
		uint32_t Size,
		FlowControlType TransferType,
		const DMA_TransferDescriptor_t *NextDescriptor)
{
	...
	ret = Chip_DMA_InitChannelCfg(&GPDMACfg, 0, src, dst, Size, TransferType);
	if (ret < 0)
		return ERROR;
...
Status Chip_DMA_SGTransfer(uint8_t ChannelNum,
		...
		FlowControlType TransferType)
{
	...
	ret = Chip_DMA_InitChannelCfg(&GPDMACfg, ChannelNum, src, dst, 0, TransferType);
	if (ret < 0)
		return ERROR;
	...
	if (IP_GPDMA_Setup(LPC_GPDMA, &GPDMACfg, dsc->ctrl, dsc->lli, SrcPeripheral, DstPeripheral) == ERROR)
		return ERROR;
