...
static uint32_t (*Otp_ProgBootSrc)(CHIP_OTP_BootSrc BootSrc);
...
uint32_t Chip_OTP_Init(void)
{
	...
	Otp_ProgBootSrc   = (uint32_t (*)(CHIP_OTP_BootSrc BootSrc))BOOTROM_API_TABLE[1];
...
uint32_t Chip_OTP_ProgBootSrc(CHIP_OTP_BootSrc BootSrc)
{
