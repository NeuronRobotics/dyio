...
static int32_t sdmmc_execute_command(uint32_t cmd, uint32_t arg, uint32_t wait_status)
{
	...
	while (step) {
		IP_SDMMC_SetClock(LPC_SDMMC, Chip_Clock_GetRate(CLK_MX_SDIO), g_card_info->speed);
...
void Chip_SDMMC_Init(void)
{
	Chip_Clock_EnableOpts(CLK_MX_SDIO, true, true, 1);
	IP_SDMMC_Init(LPC_SDMMC);
}

/* Shutdown the MCI card controller */
void Chip_SDMMC_DeInit(void)
{
	IP_SDMMC_DeInit(LPC_SDMMC);
...
int32_t Chip_SDMMC_GetDeviceBlocks(void)
{
	return g_card_info->blocknr;
}
